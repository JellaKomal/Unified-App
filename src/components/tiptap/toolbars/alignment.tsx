      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <DropdownMenuTrigger disabled={isDisabled} asChild>
            <Button variant="ghost" size="sm" className="h-8 w-max font-normal hover:bg-primary/50 hover:text-background">
              <span className="">
        ...
        <DropdownMenuGroup className=" w-40">
          {alignmentOptions.map((option, index) => (
            <DropdownMenuItem
              ...
              key={index}
              className={cn(
                "hover:bg-primary/20 hover:text-background",
                option.value === currentTextAlign() && "bg-primary/50 text-background"
              )}
            >
              <span className="mr-2">{option.icon}</span>
              {option.name}
